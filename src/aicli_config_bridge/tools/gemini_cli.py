...
from ..config import ConfigManager, GeminiCLIConfig, ToolType
...
class GeminiCLIHandler:
    ...
    def __init__(self, config_manager: ConfigManager, link_manager: LinkManager) -> None:
        ...
        self.tool_type = ToolType.GEMINI_CLI
    ...
    def default_project_paths(self) -> List[Path]:
        ...
        return [
            project_root / "configs" / "gemini-cli" / "settings.json",
        ]
    ...
    def default_context_paths(self) -> Dict[str, Path]:
        ...
        project_root = self.config_manager.project_root
        return {
            "system": Path.home() / ".gemini" / "GEMINI.md",
            "project": project_root / "GEMINI.md",
            "config_dir": project_root / ".gemini" / "GEMINI.md",
        }
    ...
    def import_config(
        ...
        if source_path is None:
            ...
            if source_path is None:
                raise FileNotFoundError("Gemini CLI の設定ファイルが見つかりません")
    ...
    def create_link(
        ...
        if project_path is None:
            ...
            if tool_config is None or tool_config.project_config_path is None:
                raise FileNotFoundError("プロジェクト内に Gemini CLI 設定が見つかりません")
    ...
    def import_context(
        ...
        if source_path is None:
            ...
            if source_path is None:
                raise FileNotFoundError("Gemini CLI のコンテキストファイルが見つかりません")
        ...
        if project_path is None:
            ...
            if tool_config is None or tool_config.project_context_path is None:
                raise FileNotFoundError(
                    "プロジェクト内に Gemini CLI コンテキストファイルが見つかりません"
                )
...
"""
    ...
    def create_default_config(self, path: Optional[Path] = None) -> GeminiCLIConfig:
        """デフォルト設定を作成.
